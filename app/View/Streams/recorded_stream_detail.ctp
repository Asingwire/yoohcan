...
<div class="right-contant">
  ...
  <div class="channels-slide streams01">
    <div class="bottom_recorded secound-option">
      ...
      <div class="strem_vdo text-center" id="dasboard_video" style="position: relative;">
		 ...
		 */?>
		    	...
		    	<?php if($recorded_stream_detail['RecordingStream']['video_play_button_type'] == "0"){
		?>
			<div class="play_pause_box" style="padding-top: 258px;">
	<a style="padding-left: 15px;" href='javascript:;' onclick="viewerStream('<?php echo $recorded_stream_detail['RecordingStream']['id'] ?>','<?php echo $recorded_stream_detail['RecordingStream']['channel_id']; ?>')">
			<?php echo $this->Html->image('Front/play_btn_new.png', array('title'=>'play','alt'=>'play','class'=>'play_pause_btn')) ?></a>
			</div>
		<?php
		}?>
		...
		<div class="row">
			<div class="col-md-12" style="padding:15px 0 0 0">
					...
					<div class="strm-dtl-top">
						<div class="user-img-div">
							<div class="usr-img">
								...
								<?php $user_name = substr($recorded_stream_detail['User']['nickname'],0,6).".";
									...
									if($image &&  file_exists(WWW_ROOT.PROFILE_IMAGE_FULL_DIR.DS.$image )) {
										echo $this->Html->image(SITE_URL.'/timthumb.php?src='.$imagePath.$image.'&w=50&h=50&a=t',array('alt'=>$user_name,'title'=>$user_name,'class'=>'user-img'));
									}
									else
									{
										echo $this->Html->image('Admin/avatar5.png', array('width'=>'50','height'=>'50','alt'=>'no image','title'=>'no image','class'=>'user-img'));
									}
								?>
				...
				<div class="col-md-2 right_discription strm_dtl_right_discription">
					<ul>
						<li  title="Total Unique Viewer" alt="Total Unique Viewer" class="viewer" id="total_viewers_count">
						<span id="viewers_count"><?php echo $recorded_stream_detail['RecordingStream']['count_viewers']?></span>
					</li>						
						...
						*/ ?>
						...
						<li  style="padding-left:9px !important;cursor:pointer !important;" title="donate" alt="donate">
						
						<?php
						if($this->Session->check('Auth.User.id') ){
						
						echo $this->Html->link($this->Html->image('Front/paypal_donate.png',array('title'=>'Donate','width'=>120,'height'=>70,'escape'=>false)),array('controller'=>'streams','action'=>'donate_for_recorded_stream',$recorded_stream_detail['RecordingStream']['id']),array('class'=>'submit_button','escape'=>false));
						
						}
						else
						{
							echo $this->Html->link($this->Html->image('Front/paypal_donate.png',array('title'=>'Donate','id'=>'donate_button_recorded_stream','width'=>120,'height'=>70,'escape'=>false)),"#",array('class'=>'submit_button','escape'=>false));
						}	
						?>
...
<script type="text/javascript">
	...
	$("#donate_button_recorded_stream").click(function() {
			$('#loginModal').modal('show');
			return false;
	});
...
<script type="text/javascript">
		...
		//Viewers Script
		function viewerStream(recording_stream_id,channel_id) { 
			
			var url = SITE_URL + 'streams/viewers_ajax';			
			var recording_stream_id = recording_stream_id;
			var channel_id = channel_id;
			// var status = '1' == 'Viewer' ? '1' : '0';
			var param1 = new Object();
			param1.user_id = '<?php echo $this->Session->read('Auth.User.id'); ?>';
			param1.recording_stream_id = recording_stream_id;
			param1.channel_id = channel_id;
			...
			$.ajax({
				...
				success: function (response) {
					obj = jQuery.parseJSON(response);					
					$("#viewers_count").text(obj.response.count);	
				}
			});
		}
		...
		/* function viewerStream(recording_stream_id,channel_id) { 
			...
			var url = SITE_URL + 'streams/viewers_ajax';
			// var value = $('.ViewersPostCl_' + recording_stream_id).attr('rel');
			// alert(value);
			var recording_stream_id = recording_stream_id;
			var channel_id = channel_id;
			var status = '1' == 'Viewer' ? '1' : '0';
